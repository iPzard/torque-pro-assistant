...
function SettingsScreen({ t, setTweak, onAddVehicle, sessions }) {
  ...
  return (
    <div className="page" data-screen-label="Settings" style={{maxWidth: 880}}>
      ...
      <Card title="OBD-II adapter" subtitle="Bluetooth hardware that bridges your vehicle to TorquePro"
            actions={<span className="pill warn"><i className="dot"/>not paired</span>}>
        <SettingRow label="Paired adapter"
                    detail="One-time hardware setup. After pairing, this adapter is trusted across all vehicle profiles."
                    last>
          <div className="row" style={{gap: 8, width: "100%", alignItems: "center"}}>
            <div className="row" style={{gap: 10, alignItems: "center", flex: 1, padding: "8px 12px",
                                          background: "var(--bg-2)", border: "1px dashed var(--border-strong)",
                                          borderRadius: 8}}>
              <span style={{width: 28, height: 28, borderRadius: 8, background: "var(--bg-3)",
                            display: "inline-flex", alignItems: "center", justifyContent: "center",
                            color: "var(--text-3)"}}>{I.vehicle}</span>
              <div>
                <div style={{fontSize: 12, color: "var(--text-2)"}}>No adapter paired yet</div>
                <div className="dim" style={{fontSize: 11, marginTop: 2}}>Vehicle profiles can't read live data until you pair one.</div>
              </div>
            </div>
            <button className="btn primary" onClick={() => window.openAdapterPairing()}>
              {I.plus}<span>Connect adapter</span>
            </button>
          </div>
        </SettingRow>
      </Card>
