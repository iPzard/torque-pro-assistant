...
function LibraryScreen({ sessions, units, onOpen, onCompare, onImport }) {
  ...
  function doExport(s) {
    setMenuFor(null);
    setToast({ kind: "ok", text: `Exported ${s.meta.fileName}` });
  }
  function doShowInFolder(s) {
    setMenuFor(null);
    setToast({ kind: "ok", text: `Revealing ${s.meta.fileName} in Finder\u2026` });
  }
  function doDelete(s) {
    setDeletedIds((p) => { const n = new Set(p); n.add(s.meta.id); return n; });
    setDeleteFor(null);
    setToast({ kind: "err", text: `Deleted \u201C${s.meta.name}\u201D` });
  }
  ...
  return (
    <div className="page" data-screen-label="Library">
      ...
      {toast && (
        <div className={"row-toast " + (toast.kind === "err" ? "err" : "ok")}>
          <span className="ico">{toast.kind === "err" ? I.trash : I.check}</span>
