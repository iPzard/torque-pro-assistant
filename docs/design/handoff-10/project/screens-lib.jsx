...
// ── Import flow ─────────────────────────────────────────────────────────────
function ImportScreen({ onCancel, onComplete, errorMode, permissionDenied, malformedRow }) {
  const [stage, setStage] = useState(errorMode ? "error" : "drop");
  const [over, setOver] = useState(false);
  const [progress, setProgress] = useState(0);
  const [filename, setFilename] = useState(errorMode ? "trackLog-20260510-093412.csv" : "");
  const [permDismissed, setPermDismissed] = useState(false);

  useEffect(() => { if (errorMode) { setStage("error"); setFilename("trackLog-20260510-093412.csv"); } }, [errorMode]);
    ...
    <div className="page" data-screen-label="Import">
      ...
      <div style={{display:"grid", gridTemplateColumns:"minmax(0, 2fr) minmax(0, 1fr)", gap:14}}>
        <div className="col" style={{gap: 14}}>
          {stage === "drop" && permissionDenied && !permDismissed && (
            <div className="alert warn" style={{marginBottom: 0}}>
              <span className="alert-icon">{I.warn}</span>
              <div className="alert-body">
                <div className="alert-title">File access denied by the operating system</div>
                <div className="alert-sub">
                  TorquePro Assistant doesn’t have permission to read files outside its sandbox. Grant access in <span className="mono" style={{color:"var(--text-1)"}}>System Settings › Privacy &amp; Security › Files &amp; Folders</span>, then try again.
                </div>
                <div className="alert-actions">
                  <button className="btn sm">{I.settings}<span>Open System Settings</span></button>
                  <button className="btn ghost sm" onClick={() => setPermDismissed(true)}>Dismiss</button>
                </div>
              </div>
              <button className="banner-close" onClick={() => setPermDismissed(true)} aria-label="Dismiss" style={{flex:"0 0 22px"}}>{I.cross}</button>
            </div>
          )}
            ...
            <div className="alert danger">
              ...
              <div className="alert-body">
                <div className="alert-title">
                  ...
                  <span className="pill err" style={{marginLeft:4}}><i className="dot"/>parse failed</span>
                </div>
                <div className="alert-sub">
                  <span className="mono" style={{color:"var(--text-0)"}}>{filename}</span> stopped at <span className="mono" style={{color:"var(--text-0)"}}>row 1,284</span> — the line has 58 columns but the header declares 67. This usually means the export was interrupted, or the file was concatenated.
                </div>
                <div className="alert-detail">
{`row 1284: 09:36:41.024,55.4,3120,42,8.6,13.4,128,37.4419,-122.143
                                              ^^^ expected 67 columns, got 58`}
                </div>
                <div className="alert-actions">
                  <button className="btn primary sm" onClick={() => startParse(filename)}>{I.reset}<span>Try again</span></button>
                  <button className="btn sm">{I.upload}<span>Pick a different file…</span></button>
                  <button className="btn ghost sm" onClick={onCancel}>Cancel</button>
                  <div className="right"/>
                  <button className="link-btn">View parser log →</button>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="col" style={{gap: 14}}>
          ...
          <Card title="Validation">
            <div className="col" style={{gap:8}}>
              ...
              {malformedRow
                ? <ValRow err label="Non-monotonic timestamps" detail={<><span>23 rows jump backwards in time — likely a clock drift during recording. They will be re-sorted on import.</span> <button className="link-btn" style={{marginLeft:6}}>view rows</button></>} />
...
function ValRow({ ok, warn, err, label, detail }) {
  const color = ok ? "var(--ok)" : warn ? "var(--warn)" : "var(--danger)";
  const icon = ok ? I.check : warn ? I.warn : I.warn;
  return (
    <div style={{display:"flex", gap:10, alignItems:"flex-start", padding:"6px 0"}}>
      <span style={{flex:"0 0 18px", color, marginTop: 1}}>{icon}</span>
      <div style={{flex:1}}>
