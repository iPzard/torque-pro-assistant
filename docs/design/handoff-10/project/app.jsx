  ...
  return (
    <div className={"app-shell" + (bannerVisible ? " has-banner" : "")}>
      ...
      <nav className="app-nav">
        ...
        {t.vehicleConnected ? (
          <button className={"nav-item" + (route.name === "vehicle-detail" ? " active" : "")}
                  onClick={() => go({ name: "vehicle-detail", id: "v1" })}>
      ...
      <TweaksPanel title="Tweaks">
        ...
        <TweakSection label="Compare states">
          ...
          <TweakButton label="Vehicle detail" onClick={() => go({ name: "vehicle-detail", id: "v1" })} />
        </TweakSection>
        ...
        <TweakSection label="Demo">
          <TweakButton label="Replay splash screen" onClick={() => window.__showSplash && window.__showSplash()} />
...
// Dismiss the boot splash once React has mounted. Keep a small floor so the
// branded boot is always visible at least briefly, even on a hot reload.
if (typeof window !== "undefined" && typeof window.__dismissSplash === "function") {
  const bootedAt = window.__splashStartedAt || performance.now();
  const elapsed = performance.now() - bootedAt;
  const floor = 1100; // ms — the splash status reel runs ~1.5s, finish it first
  setTimeout(() => window.__dismissSplash(), Math.max(0, floor - elapsed));
}
